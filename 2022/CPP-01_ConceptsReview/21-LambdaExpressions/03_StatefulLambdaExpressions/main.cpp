...
// within the reaching scope of lambda. However, it can still be accessed
...
// the captured variable within the lambda body and not the actual value
void test2() {
    ...
    std::cout<<x<<std::endl; // x=100 (original value)
...
// Only variables used within the lambda body are captured bu value
void test4() {
...
// Default capture by value and capture 'y' by reference
void test6() {
...
public:
    ...
    // Inside objects, it always captures by reference, hence from C++20, this will be
    // the correct way of use it
    auto change_person1() {
    ...
    // This will be deprecated on C++20 ownwards
    auto change_person2() {
...
// [=], [&] and [this] are equivalent when capturing an objects member variables - all captured by reference
...
void test8() {
    ...
    auto change_person1 = person.change_person1(); // This call will return a lambda function to the 'change_person1' variable
    change_person1("Moe", 30); // Calling the lambda function given
    std::cout<<person<<std::endl;
...
public:
    ...
    /* The following method returns a vector of 'Person' objects in
    'People', whose 'age > max_age' AND it limits the number of persons
    to returned to be '<=' the member variable 'max_people'.
...
void test10() {
    ...
    auto result = friends.get_people(17); // Get people who's older than 17
    for(auto &p:result) {
        std::cout<<p<<std::endl;
    }

    std::cout<<std::endl;
    friends.set_max_people(3); // Se to return only 3 people
    result = friends.get_people(17); // Get people who's older than 17
    for(auto &p:result) {
        std::cout<<p<<std::endl;
    }
    ...
    std::cout<<std::endl;
    friends.set_max_people(3); // Se to return only 3 people
    result = friends.get_people(50); // Get people who's older than 50
    for(auto &p:result) {
        std::cout<<p<<std::endl;
    }
