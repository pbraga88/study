...
int main() {
    ...
    // std::cout<<"Use count: "<<p1.use_count()<<std::endl; // use_count() displays the qty of pointers to the same location
    
    // /* With shared pointers it's possible to copy/assign (different from unique pointers) */
    // std::shared_ptr<int> p2 {p1};
    // std::cout<<"Use count: "<<p1.use_count()<<std::endl;

    // p1.reset(); // decremente use_countby setting p1 to NULL
    // std::cout<<"Use count: "<<p1.use_count()<<std::endl;
